...
<head>
    <meta charset="utf-8">
    <title>BOL{{ $boleta->numero_contrato ?? '' }}</title>
    <style>
...
<body>
    <div class="marco">
        ...
        <table>
            <tr>
                ...
                <td style="width:50%; padding:0 3mm; vertical-align:top;">
                    ...
                    <div class="sp"></div>
                    <div class="h2 azul caps">RECIBO DE DESEMPEÑO</div>
                </td>
                <td style="width:25%; vertical-align:top;">
                    <div class="t-der folio">BOL.{{ $boleta->numero_contrato ?? '' }}</div>
                    <div class="sp"></div>
                    ...
                    @if($boletaDesempeno->descuento > 0)
                    <div style="margin-top: 3mm;">
                        <div class="precioLbl" style="color: #C32020;">DESCUENTO $</div>
                        <div style="border: 0.5mm solid #C32020; padding: 1mm; font-weight: 700; text-align: center; font-size: 10pt; color: #C32020;">
                            {{ number_format($boletaDesempeno->descuento, 0, ',', '.') }}
                        </div>
                        <div style="font-size: 8pt; text-align: right; margin-top: 1mm; color: #666;">
                            Monto original: ${{ number_format($boletaDesempeno->monto_pagado + $boletaDesempeno->descuento, 0, ',', '.') }}
                        </div>
                    </div>
                    @endif
                </td>
        ...
        <table style="border: 0.5mm solid #000;">
            <tr>
                ...
                <td style="width:30%; padding: 2mm; vertical-align:top;">
                    ...
                    <div><span class="lbl">Fecha Desempeño:</span> {{ $boletaDesempeno->fecha_abono->format('d/m/Y') }}</div>
                    <div><span class="lbl">Hora:</span> {{ $boletaDesempeno->created_at->format('H:i:s') }}</div>
                    <div><span class="lbl">Atendido por:</span> {{ $boletaDesempeno->usuario->name ?? 'N/A' }}</div>
                </td>
            </tr>
        </table>
        ...
        <!-- TABLA DE DESEMPEÑOS -->
        <div class="h2 azul">HISTORIAL DE DESEMPEÑOS</div>
        <table class="tabla-cuotas">
            ...
            <tbody>
                @foreach ($todasLosDesempenos as $index => $desempenoItem)
                    <tr class="{{ $desempenoItem->id == $boletaDesempeno->id ? 'cuota-destacada' : '' }}">
                        <td class="t-centrado">{{ $index + 1 }}</td>
                        <td class="t-centrado">{{ $desempenoItem->fecha_abono->format('d/m/Y') }}</td>
                        <td class="t-der">${{ number_format($desempenoItem->monto_pagado, 0, ',', '.') }}</td>
                        <td class="t-der">
                            @if($desempenoItem->descuento > 0)
                                ${{ number_format($desempenoItem->descuento, 0, ',', '.') }}
                            @else
                                -
                            @endif
                        </td>
                        <td class="t-centrado caps">{{ $desempenoItem->observaciones }}</td>
                        <td class="t-centrado caps">{{ $desempenoItem->estado }}</td>
                    </tr>
                @endforeach
            </tbody>
        </table>
        ...
        <table style="border: 0.5mm solid #000;">
            <tr>
                <td style="width:70%; padding: 2mm; border-right: 0.5mm solid #000; vertical-align:top;">
                    <div class="resumen-item"><span class="lbl">Total de desempeños registrados:</span>
                        {{ $todasLosDesempenos->count() }}</div>
                    ...
                    <div class="resumen-item">
                        ...
                        {{ strtoupper(\App\Helpers\NumberToWordsHelper::convertir($todasLosDesempenos->sum('monto_pagado'))) }} PESOS
                    </div>
                </td>
                <td style="width:30%; padding: 2mm; vertical-align:top;">
                    <div class="resumen-item"><span class="lbl">Total monto pagado:</span>
                        ${{ number_format($todasLosDesempenos->sum('monto_pagado'), 0, ',', '.') }}</div>
                    @if($todasLosDesempenos->sum('descuento') > 0)
                    <div class="resumen-item"><span class="lbl">Total descuentos aplicados:</span>
                        ${{ number_format($todasLosDesempenos->sum('descuento'), 0, ',', '.') }}</div>
                    <div class="resumen-item"><span class="lbl">Monto original (sin desc.):</span>
                        ${{ number_format($todasLosDesempenos->sum('monto_pagado') + $todasLosDesempenos->sum('descuento'), 0, ',', '.') }}</div>
                    @endif
                    @php
                        $diasTranscurridos = $boleta->created_at->diffInDays(now());
                        $totalIntereses = $todasLosDesempenos->sum('monto_pagado') - $boleta->monto_prestamo;
                    @endphp
                    <div class="resumen-item"><span class="lbl">Total intereses:</span>
                        ${{ number_format($totalIntereses, 0, ',', '.') }}</div>
            ...
            <table>
                <tr>
                    ...
                    <td style="width:45%; text-align:center;">
                        ...
                        <div>{{ $boletaDesempeno->usuario->name ?? 'Sistema' }}</div>
                    </td>
                </tr>
            </table>
        </div>
