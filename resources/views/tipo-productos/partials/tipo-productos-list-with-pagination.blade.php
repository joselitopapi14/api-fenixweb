...
<div class="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700">
                        ...
                        <tr class="hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors duration-200">
                                ...
                                <div class="flex items-center">
                                    <div class="flex-shrink-0 h-10 w-10">
                                        <div class="h-10 w-10 rounded-full bg-gradient-to-r from-primary-600 to-primary-700 flex items-center justify-center">
                                            <x-icons.box class="h-5 w-5 text-white" />
                                        </div>
                                    </div>
                                    <div class="ml-4">
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                <div class="flex justify-end space-x-2">
                                    <a href="{{ route('tipo-productos.show', $tipo) }}"
                                    ...
                                    <a href="{{ route('tipo-productos.edit', $tipo) }}"
                                    ...
                                    <form method="POST" action="{{ route('tipo-productos.destroy', $tipo) }}" class="inline" onsubmit="return confirmDelete(event, '{{ $tipo->nombre }}')">
                                        @csrf
                                        @method('DELETE')
                                        <button type="submit"
                ...
                <div class="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 shadow-sm overflow-hidden">
                    ...
                    <div class="px-4 py-3 bg-gray-50 dark:bg-gray-700/50 border-t border-gray-200 dark:border-gray-700">
                        <div class="flex space-x-2">
                            ...
                            <form method="POST" action="{{ route('tipo-productos.destroy', $tipo) }}" class="flex-1" onsubmit="return confirmDelete(event, '{{ $tipo->nombre }}')">
                                @csrf
                                @method('DELETE')
                                <button type="submit"
    ...
    @else
        <div class="text-center py-12">
            <x-icons.box class="mx-auto h-12 w-12 text-gray-400" />
            <h3 class="mt-2 text-sm font-medium text-gray-900 dark:text-gray-100">No hay tipos de producto</h3>
            <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">Comienza creando un nuevo tipo de producto.</p>
            <div class="mt-6">
                <a href="{{ route('tipo-productos.create') }}"
...
<script>
    async function confirmDelete(event, nombreTipo) {
        ...
        const confirmed = await window.confirmDialog({
            title: '¿Estás seguro?',
            message: `El tipo de producto "${nombreTipo}" será eliminado permanentemente. Esta acción no se puede deshacer.`,
